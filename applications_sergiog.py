...
class E91Application(GeneralApplication):
    ...
    def run(self):
        ...
        # :? creo que aqui no hace falta lo de los subprotocols, borralo sin miedo

        self.start_subprotocols() # We need to run subprotocols, which are being run?
        ...
        while True:
            ...
            if self.node==first_node: # ALICE
                self.basis_set=[self.pos_basis["Z0"], self.pos_basis["Zpi_8"], self.pos_basis["Zpi_4"]]
                obs=random.choice(self.basis_set)
                meas,prob = qapi.measure(qa, observable=obs)
                #meas, prob = ns.qubits.measure(q1, observable=obs)
                print(f"{ns.sim_time():5.1f}: {self.node.name} measured "f"{meas} with probability {prob:.2f}")

                self.keydisc.append((obs.name, meas))
                
                
                
            elif self.node==last_node:

                self.basis_set=[self.pos_basis["Z0"], self.pos_basis["Zpi_8"], self.pos_basis["Zpi_m8"]]
                obs=random.choice(self.basis_set)
                meas,prob = qapi.measure(qb, observable=obs)
                #meas, prob = ns.qubits.measure(q1, observable=obs)
                print(f"{ns.sim_time():5.1f}: {self.node.name} measured "f"{meas} with probability {prob:.2f}")
                '''
                for iter in range(self.iterations):
                    
                    # Alice creates the first pair of entangled qubits
                    q1, q2 = ns.qubits.create_qubits(2)
                    ns.qubits.operate(q1, ns.H) # Apply Hadamard to the First one
                    ns.qubits.operate([q1, q2], ns.CNOT) # Apply CNOT to both, after which they are entangled
                    ...
                    self.node.ports["qubitIO"].tx_output(q2) # Send the second qubit
                    #first_node.ports[f"ccon_R_{self._path['nodes'][0]}_{self._path['request']}_teleport"].tx_output(q2)
                ...
                iter=len(message.items)
                print(f"[B] Number of iterations: {iter}")

                for i in range (iter): # TODO: Maybe we have a problem here because not all qubits got in the pkt???
                    qb=message.items[i]
                    obs=random.choice(self.basis_set)
                    meas, prob = ns.qubits.measure(qb, observable=obs)
                    print(f"{ns.sim_time():5.1f}: {self.node.name} measured "f"{meas} with probability {prob:.2f}")
                    print(message)
                    self.keydisc.append((obs.name, meas))
                '''
            ...
            if len(self.keydisc) == self.iterations:
                # End of simulation
                self.send_signal(Signals.SUCCESS, result)
